...
class Test:
    def __init__(self, params, save_dir, device, model, testset):
        self.params = params
        ...
        logger.info("# [Log]: Successfully prepared test. \n")
...
class TestInt:
    def __init__(self, params, save_dir, device, model, testset):
        ...
        logger.info("# [Log]: Successfully prepared train. \n")
    ...
    def run(self):
        ...
        logger.info(f"# [Log]: save json to ... {fname}")
...
def community_visual(signal, save_name, show=False, pdf=True):
    ...
    plt.axis('off')
    logger.info(f"# [VISUAL] => {save_name}")
    if pdf == True:
...
class TestPointCloud:
    def __init__(self, params, save_dir):
        ...
        data_path = os.path.join(
            params['mode'], config[params['mode']][params['dataset']][params['tag']]['path'])
        ...
        self.model = set_model(
            params=params, dataset=self.testset, device=self.device)  # define model
    ...
    def main_test(self):
        ...
        for i, (x, g, idx) in enumerate(self.test_loader):
            ...
            logger.info(
                f"[Test Epoch: {i}] RMSE origin: {rmse_origin:.5f}, reconst: {rmse_result:.5f}")
        ...
        # write checekpoint
        logger.info(
            f"[Test  time]: {time.time() - self.test_time:.5f} [sec], {mean_test_rmse_origin:.5f} -> {mean_test_rmse_recons:.5f}")
        logger.info(f"{self.params}")
        logger.info(
            f"  Origin: {mean_test_rmse_origin:.5f}, Reconst: {mean_test_rmse_recons:.5f}")
        self.io_model.cprint(
            f"(exp_name), {self.params['expname']}, (Origin), {mean_test_rmse_origin:.5f}, (Reconst), {mean_test_rmse_recons:.5f},")
